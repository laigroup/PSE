...
public:
	...
	DecDNNF_Manager( istream & fin , Variable max_var);
...
public: // querying
	...
	int** Decided_Num; // 这个分支决策的变量数
	vector<int>** Decide_Var;
	BigInt *Result;
	int* Vars_Num;
	set<int> Var_Set[100000];
	BigInt DecDnnf_Count_Models( NodeID u ) {
		Result = new BigInt[_nodes.Size()];
		Vars_Num = new int[_nodes.Size()];
		for (int i = 0 ; i < _nodes.Size() ; i ++) {
			Result[i] = -1;
			Vars_Num[i] = 0;
			Var_Set[i].clear();
		}
		BigInt res = Count_Models_DecDnnf(u);
		delete[] Result;
		delete[] Vars_Num;
		return res;
	}
	BigInt Count_Models_DecDnnf( NodeID root );
...
public:
	static void Debug()
	{
		...
		DecDNNF_Manager cdd_manager( fin , Variable(10)); // 
		fin.close();
