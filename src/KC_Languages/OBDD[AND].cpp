...
_nodes( LARGE_HASH_TABLE )
{
	...
	*/
	...
	// thl //if ( !String_Fuzzy_Match( p, "F 0" ) ) cerr << "ERROR[BDDC]: wrong BDDC-file format, wrong false-node!" << endl;
	...
	//thl if ( !String_Fuzzy_Match( p, "T 0" ) ) cerr << "ERROR[BDDC]: wrong BDDC-file format, wrong true-node!" << endl;
...
double OBDDC_Manager::Count_Entropy( const Diagram & bddc , vector<int>& belong_XORY) {
	NodeID root = bddc.Root();
	unsigned num_vars = NumVars( _max_var );
	BigInt result;
	if ( Is_Fixed( root ) ) {
	    if ( root == NodeID::bot ) return 0;
	...
	unsigned num_node_stack = 1;
	BigInt * results = new BigInt [root + 1];
	double *entropy = new double [root + 1];
	...
	while ( num_node_stack ) {
		...
		if ( topn.infor.mark != UNSIGNED_UNDEF ) {
			...
			if ( _node_mark_stack[num_node_stack - 1] ) {
				...
				BDDC_Node & low = _nodes[topn.ch[0]];
				BDDC_Node & high = _nodes[topn.ch[1]];
				if (belong_XORY[_nodes[top].sym]) {
					double low_models = results[topn.ch[0]].TransformDouble() ;
					//low_models.Mul_2exp(low.infor.mark);
					double high_models = results[topn.ch[1]].TransformDouble() ;
					//high_models.Mul_2exp(high.infor.mark);
					//cout << "results[" << top << "] = " << results[top] << " * 2 ^ " << topn.infor.mark << endl;
					int low_pw = low.infor.mark;
					int high_pw = high.infor.mark; 
					double p_low = 1.0 / (1 + high_models / low_models  * pow(2.0,high_pw - low_pw));
					double p_high = 1.0 / (1 + low_models / high_models * pow(2.0,low_pw - high_pw));
					//cout<<" lowm = "<<low_models<<" highm = "<<high_models<<" ";
					//cout<<" var = "<<_nodes[top].sym<<" plow = "<<p_low<<" phigh = "<<p_high<<endl;
					//cout<<" entropy[ch0] = "<<entropy[topn.ch[0]]<<" entropy[ch1] = "<<entropy[topn.ch[1]]<<endl;
					if (p_low != 0) entropy[top] += p_low * entropy[topn.ch[0]] - p_low * log2(p_low);
					if (p_high != 0) entropy[top] += p_high * entropy[topn.ch[1]] - p_high * log2(p_high); // to check
					//cout<<" entropy = "<<entropy[top]<<endl;
				}
				if ( low.infor.mark < high.infor.mark ) {
					results[top] = results[topn.ch[1]];
					results[top].Mul_2exp( high.infor.mark - low.infor.mark );
			...
			if ( loc == topn.ch_size ) {
				...
				num_node_stack--;
				results[top] = results[topn.ch[loc]];
				entropy[top] = entropy[topn.ch[loc]]; // thl add
				topn.infor.mark = _nodes[topn.ch[loc]].infor.mark;
				//cout<<" [AND] entropy = "<<entropy[top]<<endl;
                for ( unsigned i = loc + 1; i < topn.ch_size; i++ ) {
                    results[top] *= results[topn.ch[i]];
					entropy[top] += entropy[topn.ch[i]]; // thl add
					//cout<<" [AND] entropy = "<<entropy[top]<<" entropy[ch] = "<<entropy[topn.ch[i]]<<endl;
                    topn.infor.mark += _nodes[topn.ch[i]].infor.mark;
                }
                topn.infor.mark -= ( topn.ch_size - loc - 1 ) * num_vars + loc;
				_visited_nodes.push_back( top );
	...
	_visited_nodes.clear();
	double Entropy_Res = entropy[root];
	delete [] results;
	delete [] entropy;
	return Entropy_Res;
}
BigInt OBDDC_Manager::Count_Models( const Diagram & bddc, const vector<Literal> & assignment )
{
	...
	for ( i = 0; !Lit_UNSAT( assignment[i] ); i++ ) {
		...
		_assignment[assignment[i].Var()] = assignment[i].Sign();
		//cout<<assignment[i].Var()<<" sign = "<<assignment[i].Sign()<<endl;
	}
	//cout<<" i = "<<i<<endl;
	Un_Label_Value( assignment.back() );
	BigInt result;
