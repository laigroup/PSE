...
#define CDD_SYMBOL_Decision  (unsigned(-6))
DecDNNF_Manager::DecDNNF_Manager( istream & fin , Variable max_var ):
CDD_Manager( Variable::start, LARGE_HASH_TABLE )
{
	...
	cout<<" here "<<endl;
	vector<int>sym; // 0 false 1 true 2 or 3 and
	vector<CDD_Node> Nodes;
	vector<int>Index_Nodes;
	vector<int>rd;
	vector<int> graph[max_var * 100];
	vector<int> tmp_decided_num[max_var * 100];
	vector<int>** tmp_var_decide;
	Decided_Num = new int*  [max_var * 100];
	Decide_Var = new vector<int>* [max_var * 100];
	tmp_var_decide = new vector<int>* [max_var * 100];
	Index_Nodes.resize(max_var * 100);
	Nodes.resize(max_var * 100);
	sym.resize(max_var * 100);
	rd.resize(max_var * 100);
	for (int i = 0 ; i < max_var * 100 ; i ++) {
		Index_Nodes[i] = 0;
		graph[i].clear();
		tmp_decided_num[i].resize(3);
		Decided_Num[i] = new int[3];
		Decide_Var[i] = new vector<int> [3];
		tmp_var_decide[i] = new vector<int> [3];
		for (int j = 0 ; j <= 1 ; j ++) {
			Decided_Num[i][j] = tmp_decided_num[i][j] = 0;
		}
		Nodes[i].ch = nullptr;
		Nodes[i].ch_size = 0;
		Nodes[i].infor.mark = 0; //.Init();
		rd[i] = 0;
	}
	...
	unordered_map<int,NodeID> mp;
	mp.clear();
	while(fin >> op) {
		if (fin.eof()) break;
		cout<<"op = "<<op<<endl;
		if (op[0] == 'o' || op[0]  == 't' || op[0]  == 'f' || op[0]  == 'a') {
			int x;
			int idx = -1;
			bool flag = false;
			while(fin >> x && x) {
				if (idx == -1) {
					idx = x;
					if (op[0] == 'f') {
						Nodes[idx].sym = CDD_SYMBOL_FALSE;
						sym[idx] = 0;
					}
					else if (op[0]  == 't') {
						Nodes[idx].sym = CDD_SYMBOL_TRUE;
						sym[idx] = 1;
					}
					else if (op[0]  == 'o') {
						Nodes[idx].sym = CDD_SYMBOL_Decision;
						sym[idx] = 2;
					}
					else {
						assert(op[0]  == 'a');
						Nodes[idx].sym = CDD_SYMBOL_DECOMPOSE; //? or conjion?
						sym[idx] = 3;
					}
				}
				else if (!flag) {
					flag = true;
					cout<<" IDx = "<<idx<<" size = "<<x<<endl;
					Nodes[idx].ch_size = x;
				}
			}
			if (op[0] == 'f') mp[idx] = 0;
			...
			while(fin >> x && x) {
				...
				if (cnt >= 2) {
					implied_var_num ++;
					if (Nodes[u].ch_size == 0 || u == 1) tmp_var_decide[u][0].push_back(abs(x));
					else tmp_var_decide[u][1].push_back(abs(x));
				}
			}
			if (Nodes[u].ch == nullptr) {
				Nodes[u].ch = new NodeID [30]; // Nodes[u].ch_size
				for (int i = 0 ; i < 30 ; i ++) { // Nodes[u].ch_size
					Nodes[u].ch[i] = 0;
				}
			}
			cout<<" u = "<<u<<" Decide_Var[u][0] = "<<tmp_var_decide[u][0].size()<<" Decide_Var[u][1] = "<<tmp_var_decide[u][1].size()<<" implied_var_num = "<<implied_var_num<<endl;
			if (sym[u] == 2) {
				Nodes[u].ch_size ++;
				if (cnt < 2) {
					Nodes[u].sym = 0;
				...
				else if (Nodes[u].ch_size == 1 || u == 1) {
					Nodes[u].ch[0] = v;
					tmp_decided_num[u][0] = implied_var_num;
				}
				else {
					Nodes[u].ch[1] = v;
					tmp_decided_num[u][1] = implied_var_num;
				}
				Nodes[u].sym = abs(var_sym);

				cout<<" nodes"<<u<<" sym = "<<abs(var_sym)<<endl;
			}
			else {
				assert(sym[u] == 3);
				cout<<" size = "<<Nodes[u].ch_size<<endl;
				Nodes[u].ch[Nodes[u].ch_size ++] = v;
				cout<<" u = "<<u<<" v = "<<v<<endl;
			}
			graph[v].push_back(u);
			rd[u] ++;
	...
	cout<<" maxidx = "<<max_idx<<endl;
	queue<int>q;
	for (int i = 1 ; i <= max_idx ; i ++) {
		cout<<" i = "<<i<<" rd = "<<rd[i]<<endl;
		if (!rd[i]) q.push(i);
	}
	while(!q.empty()) {
		int u = q.front();
		cout<<" u = "<<u<<" size=  "<<Nodes[u].ch_size<<endl;
		for (int i = 0 ; i < Nodes[u].ch_size ; i ++) {
			if (mp.find(Nodes[u].ch[i]) != mp.end()) {
				//cout<<" u = "<<u<<" ch = "<<Nodes[u].ch[i]<<" mp = "<<mp[Nodes[u].ch[i]]<<endl;
				Nodes[u].ch[i] = mp[Nodes[u].ch[i]];
			}	
			cout<<" u = "<<u<<" ch = "<<Nodes[u].ch[i]<<endl;
		}
		CDD_Node node = Nodes[u];
		node.Display(cout);
		NodeID ID; 
		if (mp.find(u) != mp.end()) ID = mp[u];
		else ID = Push_Node( node );
		_nodes[ID].infor.mark = Nodes[u].infor.mark;
		mp[u] = ID;
		if (Nodes[u].sym > 0 && Nodes[u].sym <= max_var) {
			Decided_Num[ID][0] = tmp_decided_num[u][0];
			Decided_Num[ID][1] = tmp_decided_num[u][1];

			Decide_Var[ID][0] = tmp_var_decide[u][0];
			Decide_Var[ID][1] = tmp_var_decide[u][1];

			cout<<" u = "<<u<<" ID = "<<ID<<" low "<<Decided_Num[ID][0]<<" high = "<<Decided_Num[ID][1]<<endl;
		}
...
CDD_Manager( Variable::start, LARGE_HASH_TABLE )
{
	...
	// thl if ( !String_Fuzzy_Match( p, "F 0" ) ) cerr << "ERROR[CDD]: wrong CDD-file format, wrong false-node!" << endl;
	...
	// thl if ( !String_Fuzzy_Match( p, "T 0" ) ) cerr << "ERROR[CDD]: wrong CDD-file format, wrong true-node!" << endl;
...
BigInt DecDNNF_Manager::Count_Models_DecDnnf( NodeID u ){
	if (u == NodeID::top) {
		unsigned num_vars = NumVars( _max_var );
		BigInt res = 1;
		res.Mul_2exp(_max_var);
		return res;
	}
	if (u == NodeID::bot) return 0;
	if (Result[u] != -1) return Result[u];
	BigInt res = 0;
	for (int i = 0 ; i < _nodes[u].ch_size ; i ++) {
		NodeID ch = _nodes[u].ch[i];
		BigInt ch_res = Count_Models_DecDnnf(ch);
		for (auto j : Var_Set[ch]) {
			Var_Set[u].insert(j);
		}
		if (_nodes[u].sym != CDD_SYMBOL_DECOMPOSE)
		for (int j = 0 ; j < Decide_Var[u][i].size() ; j ++) {
			Var_Set[u].insert(Decide_Var[u][i][j]);
		}
		//cout<<" Decide_Var[u][i].size() = "<<Decide_Var[u][i].size()<<" Decided_Num[u][i] = "<<Decided_Num[u][i]<<endl;
		if (_nodes[u].sym != CDD_SYMBOL_DECOMPOSE) {
			cout<<" Decide_Var[u][i].size() = "<<Decide_Var[u][i].size()<<" Decided_Num[u][i] = "<<Decided_Num[u][i]<<endl;
			assert(Decide_Var[u][i].size() == Decided_Num[u][i]);
			cout<<" u = "<<u<<" sym = "<<_nodes[u].sym<<endl;
			//if (u != (NodeID)_nodes.Size() - 1) assert(Decided_Num[u][i] > 0);
		}
		if (_nodes[u].sym == CDD_SYMBOL_DECOMPOSE) {
			if (res == 0) res = 1;
			res *= ch_res;
			if (i >= 1) res.Div_2exp(_max_var);
		} 
		else {
			ch_res.Div_2exp(Decided_Num[u][i]);
			res += ch_res;
		}
	}
	// for (auto i : Var_Set[u]) {
	// 	cout<<" node u var = "<<i<<endl;
	// }
	cout<<"max_var = "<<_max_var<<endl;
	//if (_nodes[u].sym == CDD_SYMBOL_DECOMPOSE) res.Div_2exp(_max_var * (_nodes[u].ch_size-1));
	cout<<" u = "<<u<<" sym = "<<_nodes[u].sym<<" res = "<<res<<" setsize = "<<Var_Set[u].size()<<" low_implied = "<<Decided_Num[u][0]<<" high_implied = "<<Decided_Num[u][1]<<endl;
	Result[u] = res;
	return res;
}
