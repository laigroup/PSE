...
void Preprocessor::Count_Yimplied( CNF_Formula & cnf ) {
	vector<unsigned>Ylist = cnf.Get_Ylist();
	for (auto it = cnf.Clause_Begin() ; it != cnf.Clause_End() ; it ++) {
		Clause cls = *it;
		//cls.Display(cout);
	}
	Literal * lits = new Literal [5 * _max_var + 1];
	Y_implied = new bool [3 * _max_var + 1];
	...
	for (auto it = cnf.Clause_Begin() ; it != cnf.Clause_End() ; it ++) {
		...
		for (int i = 0 ; i < cls.Size() ; i ++) {
			...
			int lit_var = lit.Var();
			lit_var *= sign;
			tmp.push_back(lit_var);
			int var = lit.Var() + _max_var;
			tmp2.push_back(var * sign);
			//cout<<" Xi = "<<lit<<" Yi = "<<lits[len - 1]<<endl;
		}
	...
	unsigned pre_max_var = _max_var;
	//vector<Model *> premodel = _models_stack[0];
	//Display_Models(premodel,cout);

	while(1) {
		bool find_implied = false;
		vector< vector<int> > Cluase_test = clause_tmp;
		vector<int> cls_tmp;
		unsigned best = 0;
		for (int i = 0 ; i < Ylist.size() ; i ++) { // 尝试看看Y中剩余变量是否能蕴含Ylist[i]
			if (Y_implied[i]) continue; // ? 需不要呢
			Cluase_test = clause_tmp;
			for (int j = 0 ; j < Ylist.size() ; j ++) {
				if (Y_implied[j]) continue;
				int var_a = Ylist[j];
				int var_b = Ylist[j] + _max_var;
				Clause cls;
				if (i == j) { 
					cls_tmp.clear();
			...
			//Display_Models(_models_stack[0],cout);
			//_models_stack[0] = premodel;
			_num_levels = 0;
			_num_dec_stack = 0;
			if (!Test_sat) {
				//cout<<i<<" ";
				...
				if (!best) {
					...
					// if ( _var_order.Less_Than( Ylist[best], Ylist[i] ) ) {
					// 	best = i;
					// }
				}
				//cout<<" Implied Y "<<Ylist[i]<<endl;
			}
		}
		if (!find_implied) break;
		//else Y_implied[best] = true;
		//cout<<"   best = "<<best;
		//cout<<endl;
	}
	...
	// cout<<"Y implied Finish" <<endl;
	
	cout<<"c ind ";
	for (int i = 0 ; i < Ylist.size() ; i ++) {
		//cout<<" i = "<<i<<" Y_implied = "<<Y_implied[i]<<endl;
		if (Y_implied[i]) continue;
		cout<<Ylist[i]<<" ";
		tt.push_back(Ylist[i]);
		belong_XORY[Ylist[i]] = 1;
	}
	cout<<0<<endl;
	cnf.Clear_Ylist();
	cnf.Copy_Ylist(tt);
	...
	// 	vector<int> cls_tmp;
	// 	unsigned best = 0;
	// 	for (int i = 1 ; i <= cnf.Max_Var() ; i ++) { // 尝试看看Y中剩余变量是否能蕴含Ylist[i]
	// 		if (Y_implied[i]) continue; 
	// 		Cluase_test = clause_tmp;

	// 		Variable var_a = Variable(i);
	// 		Variable var_b = Variable(i + _max_var);
	// 		cls_tmp.clear();
	// 		cls_tmp.push_back(var_a * -1);
	// 		cls_tmp.push_back(var_b * -1);
	// 		Cluase_test.push_back(cls_tmp);
	...
	// 		for (int j = 0 ; j < Ylist.size() ; j ++) {
	// 			//if (Y_implied[j]) continue;
	// 			Variable var_a = Variable(Ylist[j]);
	// 			Variable var_b = Variable(Ylist[j] + _max_var);
	// 			Clause cls;
	// 			cls_tmp.clear();
	// 			cls_tmp.push_back(var_a * -1);
	// 			cls_tmp.push_back(var_b);
	...
	// 			Cluase_test.push_back(cls_tmp);
	// 		}
	// 		bool Test_sat = Solve_SAT( _models_stack[0] , Cluase_test);
	// 		//Display_Models(_models_stack[0],cout);
	// 		_models_stack[0] = premodel;
	// 		_num_levels = 0;
	// 		_num_dec_stack = 0;
...
bool Preprocessor::Preprocess( CNF_Formula & cnf, vector<Model *> & models )
{
	...
	if ( !sat ) {
		...
		//running_options.display_preprocessing_process = true;
		if ( running_options.display_preprocessing_process ) {
...
bool Preprocessor::Preprocess( vector<Model *> & models )
{
	...
	_fixed_num_vars = _unary_clauses.size();
	// cout<<"fix_num_vars = "<<_fixed_num_vars<<endl;
	if (running_options.Pre) {
		do {
	...
	//Display_Lit_Equivalence(cout);
	// for ( Literal lit = Literal::start; lit <= 2 * _max_var + 1; lit++ ) {
	// 	if ( _lit_equivalences[lit] != lit ) {
	// 		if (belong_XORY[lit.Var()]) belong_XORY[_lit_equivalences[lit].Var()] = 1;
	// 	}
	// } // thl add 将Y对应的等价文字 变成Y


	if ( running_options.Pre && running_options.recover_exterior ) Transform_Exterior_Into_Clauses();
...
void Preprocessor::Eliminate_Redundancy() // 消除冗余
{
...
bool Preprocessor::Replace_Equivalent_Lit()
{
	...
	begin_watch.Start();
	//if ( !Detect_Lit_Equivalence() ) { // precode
	if (!Detect_Lit_Equivalence_BCP()) {
		if ( running_options.profile_preprocessing >= Profiling_Detail ) statistics.time_replace_lit_equivalences += begin_watch.Get_Elapsed_Seconds();
		return false;
	} 
	for ( Variable i = Variable::start; i <= _max_var; i++ ) {
	...
	Replace_Equivalent_Lit_Binary_Clauses(); // thl modify
	Store_Long_Learnts(); 
	Replace_Equivalent_Lit_Long_Clauses(); // thl modify
	_old_num_long_clauses = _long_clauses.size();
	for ( Variable i = Variable::start; i <= _max_var; i++ ) {
		_old_num_binary_clauses[i + i] = _binary_clauses[i + i].size();
		_old_num_binary_clauses[i + i + 1] = _binary_clauses[i + i + 1].size();
	}
	Replace_Equivalent_Lit_Binary_Learnts(); // thl modfiy
	Replace_Equivalent_Lit_Long_Learnts(); // thl modify
...
void Preprocessor::Cluster_Equivalent_Lits() // thl modify
{
	// _fixed_num_vars = _unary_clauses.size() + _and_gates.size(); 
	// _equivalent_lit_cluster_size = 0;
	// vector<Literal> singleton( 1 );
	// for ( Variable i = Variable::start; i <= _max_var; i++ ) { 
	// 	Literal lit = Literal(i + i);
	// 	Literal root = Find_Equivalent_Lit_Root(Literal(i + i)); 
	// 	//_lit_equivalences[lit] = root; // thl
	// 	if ( root == lit ) {  // NOTE: lit may be changed, so we need to use _lit_equivalences[i + i]
	...
	// 	Literal root = Find_Equivalent_Lit_Root(Literal(i + i));
	// 	if ( root != Literal(i + i)) {  // NOTE: lit may be changed, so we need to use _lit_equivalences[i + i]
	// 		unsigned j;
	// 		for ( j = 0; _lit_equivalences[i + i] != _equivalent_lit_sets[j][0]; j++ ) {}  // find the old entry
	// 		assert(_lit_equivalences[i + i] == _equivalent_lit_sets[j][0]);
			
	// 		// if (belong_XORY[i] == 1 && belong_XORY[root.Var()] == 1) {
	// 		// 	continue;
	// 		// }
	// 		_equivalent_lit_sets[j].push_back( Literal( i, false ) );
	// 		_equivalent_lit_sets[j ^ 0x01].push_back( Literal( i, true ) );
	// 		//if (belong_XORY[i] == 0) _fixed_num_vars++;  // thl 只有X能作为fixnum Y不行
	// 		_fixed_num_vars++; // precode
	// 	}
	// }
	// for ( Variable i = Variable::start; i <= _max_var; i++ ) {
	// 	Literal lit = Find_Equivalent_Lit_Root(Literal(i,false));
	// 	lit = Find_Equivalent_Lit_Root(Literal(i,true));
	// }
...
void Preprocessor::Replace_Equivalent_Lit_Binary_Clauses() 
{
	unsigned i, j, k;
	for ( i = 0; i < _equivalent_lit_cluster_size; i++ ) { 
		// 把根 对应的文字 的二元文字 都加入到根的二元子句中
		Literal root = _equivalent_lit_sets[i][0];
		for ( k = 0; k < _binary_clauses[root].size(); k++ ) { // 把root的二元文字 变成这个二元文字 对应的等价文字
			Literal equ_lit = _lit_equivalences[_binary_clauses[root][k]];
			//Variable var = _binary_clauses[root][k].Var(); // thl
			// if (belong_XORY[var] == 1 && belong_XORY[equ_lit.Var()] == 1) { // thl 不可以Y->Y
				
			// 	if ( _lit_seen[_binary_clauses[root][k]] ) { 
			// 		Simply_Erase_Vector_Element( _binary_clauses[root], k );
			// 		k--;
			// 	}
			// 	else _lit_seen[_binary_clauses[root][k]] = true;

			// 	//_binary_clauses[root][k] = _binary_clauses[root][k]; // 不变
			// 	continue;
			// } 
			
			if ( equ_lit.Var() == root.Var() || _lit_seen[equ_lit] ) { 
				// 如果二元文字_binary_clauses[root][k] 的等价文字 也是root,则可以删掉这个二元文字了
				// 或者如果二元文字的等价类出现过了，说明已经被放入root的二元子句中了，也可以删了
				Simply_Erase_Vector_Element( _binary_clauses[root], k );
				k--;
			}
			else {
		...
		for ( j = 1; j < _equivalent_lit_sets[i].size(); j++ ) {
			Literal lit = _equivalent_lit_sets[i][j];
			assert( root == _lit_equivalences[lit] ); // 这些等价文字类 都会等价于 root 
			for ( k = 0; k < _binary_clauses[lit].size(); k++ ) { 
				Literal equ_lit = _lit_equivalences[_binary_clauses[lit][k]];
				if ( equ_lit.Var() != root.Var() && !_lit_seen[equ_lit] ) { 
					// 如果lit的二元文字 对应的等价文字不是root ，并且这个等价文字没出现过，则替换掉该二元文字
					_binary_clauses[root].push_back( equ_lit ); 
					_lit_seen[equ_lit] = true;
				}
			}
			//if (belong_XORY[lit.Var()] == 1) continue; // thl Y的二元子句不能删除
			_binary_clauses[lit].clear(); // 这个删了也无所谓？
		}
		for ( j = 0; j < _binary_clauses[root].size(); j++ ) {
			_lit_seen[_binary_clauses[root][j]] = false;
		}
...
void Preprocessor::Replace_Equivalent_Lit_Long_Clauses()
{
	unsigned i;
	vector<Clause>::iterator begin = _long_clauses.begin(), itr;
	for ( itr = begin; itr < _long_clauses.end(); itr++ ) { // 核心思想是 将长子句里的每个文字都替换成对应的等价文字
		_big_clause.Clear(); // 在这里存储没出现过的等价文字
		for ( i = 0; i < itr->Size(); i++ ) {
			Literal equ_lit = _lit_equivalences[(*itr)[i]]; // 取出对应的等价文字
			// Literal lit = (*itr)[i];  // thl
			// if (belong_XORY[lit.Var()] == 1) { // thl 不允许 Y->Y X->Y
			// 	if ( _lit_seen[lit] ) continue; // 如果出现过 跳过
			// 	else if ( _lit_seen[~lit] ) break; // 重言式
			// 	else {
			// 		_big_clause.Add_Lit( lit ); // Y原封不动
			// 		_lit_seen[lit] = true;
			// 	}
			// 	continue;
			// }
			if ( _lit_seen[equ_lit] ) continue; // 如果等价文字出现过 跳过
			else if ( _lit_seen[~equ_lit] ) break; // 如果等价文字的非 出现过 结束循环？ 说明出现了 equ lit \vee ~equ lit?
			else {
				_big_clause.Add_Lit( equ_lit ); // 等价文字没出现过，加入到big clause里
				_lit_seen[equ_lit] = true; // 标记出现
			}
		}
		if ( i < itr->Size() ) {  // Annotate: tautology 重言式 永真式 equ_lit \vee ~equ_lit
			_long_clauses[itr - begin].Free();
			Simply_Erase_Vector_Element( _long_clauses, itr - begin );
			itr--;
			_lit_seen[_big_clause[0]] = false;
			for ( i = 1; i < _big_clause.Size(); i++ ) _lit_seen[_big_clause[i]] = false; // 还原
			continue;
		}
		assert( _big_clause.Size() > 1 );
		_lit_seen[_big_clause[0]] = false;
		_lit_seen[_big_clause[1]] = false; 
		if ( _big_clause.Size() == 2 ) { // 说明这个长子句替换等价文字之后变成二元子句了 将两个文字都替换成对应的等价文字 后添加新的二元子句
			Add_Binary_Clause_Naive( _big_clause[0], _big_clause[1] ); 
			_long_clauses[itr - begin].Free(); // 同时删除这个长子句
			Simply_Erase_Vector_Element( _long_clauses, itr - begin );
			itr--;
		}
		else { // 不是二元子句 说明是 > 2 的长子句 big_clause 覆盖 长子句
			(*itr)[0] = _big_clause[0];
			(*itr)[1] = _big_clause[1];
			for ( i = 2; i < _big_clause.Size(); i++ ) { 
				_lit_seen[_big_clause[i]] = false; // 还原lit_seen
				(*itr)[i] = _big_clause[i]; // 将big clause 覆盖原来的长子句
			} 
			itr->Shrink( _big_clause.Size() ); // 更改大小
			_long_watched_lists[(*itr)[0]].push_back( itr - begin );
			_long_watched_lists[(*itr)[1]].push_back( itr - begin );
		}
	}
...
void Preprocessor::Replace_Equivalent_Lit_Binary_Learnts()
{
	...
	for ( i = 0; i < _equivalent_lit_cluster_size; i++ ) { 
		Literal root = _equivalent_lit_sets[i][0];
		for ( j = 0; j < _binary_clauses[root].size(); j++ ) { // 先将root对应的二元文字标记为已经出现
			_lit_seen[_binary_clauses[root][j]] = true;
		}
		for ( k = 0; k < _binary_learnts[root].size(); k++ ) {
			Literal equ_lit = _lit_equivalences[_binary_learnts[root][k]];
			if ( equ_lit.Var() != root.Var() && !_lit_seen[equ_lit] ) { // 如果未出现该二元文字 且对应的等价文字不是root 则加入root的二元子句中
				_binary_clauses[root].push_back( equ_lit );
				_lit_seen[equ_lit] = true;
			}
		}
		_binary_learnts[root].clear(); // thl 要不要改呢？
		for ( j = 1; j < _equivalent_lit_sets[i].size(); j++ ) { 
			Literal lit = _equivalent_lit_sets[i][j];
			assert( root == _lit_equivalences[lit] );
			for ( k = 0; k < _binary_learnts[lit].size(); k++ ) {
				Literal equ_lit = _lit_equivalences[_binary_learnts[lit][k]];
				if ( equ_lit.Var() != root.Var() && !_lit_seen[equ_lit] ) { // 这个好像没什么问题？
					_binary_clauses[root].push_back( equ_lit );
					_lit_seen[equ_lit] = true;
				}
			}
			_binary_learnts[lit].clear(); // thl 要不要改呢 ？ 不改好像也没毛病啊？
		}
		for ( j = 0; j < _binary_clauses[root].size(); j++ ) {
			_lit_seen[_binary_clauses[root][j]] = false;
		}
...
void Preprocessor::Replace_Equivalent_Lit_Long_Learnts() // 在长学习字句中替换等价文字
{
	vector<Clause>::iterator begin, itr;
	for ( itr = begin = _long_learnts.begin(); itr < _long_learnts.end(); itr++ ) {
		_big_clause.Clear();
		...
		for ( i = 0; i < itr->Size(); i++ ) {
			...
			// if (belong_XORY[lit.Var()] == 1) { // thl 不允许 Y->Y X->Y
			// 	if ( _lit_seen[lit] ) continue; // 如果等价文字出现过 跳过
			// 	else if ( _lit_seen[~lit] ) break; // 重言式
			// 	else {
			// 		_big_clause.Add_Lit( lit ); // Y原封不动
			// 		_lit_seen[lit] = true;
...
Literal Preprocessor::Find_Equivalent_Lit_Root( Literal lit ) {
	if (_lit_equivalences[lit] == lit) return lit;
	return _lit_equivalences[lit] = Find_Equivalent_Lit_Root(_lit_equivalences[lit]);
}
void Preprocessor::Record_Equivalent_Lit_Pair( Literal lit, Literal lit2 )
{
	// 并查集 合并等价文字
	
	// Literal root1 = Find_Equivalent_Lit_Root(lit);
	// Literal root2 = Find_Equivalent_Lit_Root(lit2);
	// Variable var1 = root1.Var();
	// Variable var2 = root2.Var();
	// if (belong_XORY[var1] == 1) {
	// 	_lit_equivalences[root2] = root1;
	//  	_lit_equivalences[~root2] = ~root1;
	// }
	// else {
	// 	_lit_equivalences[root1] = root2;
	//  	_lit_equivalences[~root1] = ~root2;
	// }
...
void Preprocessor::Transform_Exterior_Into_Clauses()
{
	...
	for ( Variable i = Variable::start; i <= _max_var; i++ ) {
		...
		if ( _lit_equivalences[lit] != lit ) {  /// NOTE: no need to compare lit + 1
			//if (belong_XORY[lit.Var()] == 0) continue; // thl
