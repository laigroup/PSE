...
void KCounter::Allocate_and_Init_Auxiliary_Memory( Variable max_var )  // ToDo: whether can we optimize when max_var < _max_var
{
	...
	Counting_stack = new BigInt [2 * _max_var + 2]; // thl
	Entropy_stack = new double [2 * _max_var + 2]; // thl
	_aux_rsl_stack = new unsigned [2 * _max_var + 2];
	//belong_XORY.resize(_max_var * 2 + 2); // thl
}
...
BigInt KCounter::Count_Models( CNF_Formula & cnf, Heuristic heur )
{
	...
	// for ( int i = 0; i < _old_num_long_clauses; i ++) {
	// 	for ( int j = 0; j < _long_clauses[i].Size(); j ++) {
...
//				Display_Statistics( 0 );
	...
	// heur = minfill;
	// for (Variable i = Variable::start ; i <= _max_var ; i ++) {
	// 	cout<<"i = "<<i<<" apperar is "<<Var_Appeared(i)<<" ";
	// 	cout<<"binary size = "<<_binary_var_membership_lists[i].size()<<" ";
	// 	cout<<"ternary size =  "<<_ternary_var_membership_lists[i].size()<<" ";
	// 	cout<<"quaternary size =  "<<_quaternary_var_membership_lists[i].size()<<" ";
	// 	cout<<"long var size = "<<_long_var_membership_lists[i].size()<<endl;
	// }
	...
	if ( debug_options.verify_count ) { // 
		BigInt verified_count = Count_Verified_Models_sharpSAT(cnf);// Count_Verified_Models_d4( cnf );
...
void KCounter::Create_Init_Level()
{
	...
	_active_comps[1] = 0;
	_num_levels = 2;
	//assert( _component_cache.Size() == 0 ); pre code
...
BigInt KCounter::Backtrack_Init()
{
	if ( _num_rsl_stack == 0 ) {
		...
		//cout<<"_num_rsl_stack = "<<_num_rsl_stack<<" num_level = "<<_num_levels<<endl;
		_num_rsl_stack--;
		//cout<<"res = "<<_rsl_stack[0]<<endl;
...
void KCounter::Choose_Running_Options( Heuristic heur )
{
	...
	case SharpSAT_TD:
		Compute_Var_Order_Automatical();
		running_options.var_ordering_heur = SharpSAT_TD;
		break;
...
void KCounter::Compute_Var_Order_Automatical()
{
	...
	//cout<<" _unsimplifiable_num_vars = "<<_unsimplifiable_num_vars<<endl;
	...
	//cout<<running_options.treewidth<<","<<treewidth_bound<<",";
	
	//cout<<"treewidth = "<<running_options.treewidth<<" treewidth_bound = "<<treewidth_bound<<endl;
...
void KCounter::Component_Cache_Add_Original_Clauses()
{
	unsigned j, num;
	for ( Variable i = Variable::start; i <= _max_var; i++ ) {
		if (Var_Decided(i)) continue; // now code
		Literal lit( i, false );
		for ( j = num = 0; j < _old_num_binary_clauses[lit]; j++ ) {
			Literal lit2 = _binary_clauses[lit][j];
			if (Var_Decided(lit2.Var())) continue; // now code
			if ( lit > lit2 ) continue;
			_many_lits[num++] = lit2;
		}
		_qsorter.Sort( _many_lits, num );
		for ( j = 0; j < num; j++ ) {
			// assert( Lit_Undecided( lit ) && Lit_Undecided( _many_lits[j] ) ); pre code
			if ( Lit_Undecided( lit ) && Lit_Undecided( _many_lits[j] ) )
		...
		_qsorter.Sort( _many_lits, num );
		for ( j = 0; j < num; j++ ) {
			// assert( Lit_Undecided( lit ) && Lit_Undecided( _many_lits[j] ) ); precode
			if ( Lit_Undecided( lit ) && Lit_Undecided( _many_lits[j] ) )
			_component_cache.Add_Original_Binary_Clause( lit, _many_lits[j] );
		}
	}
...
void KCounter::Count_With_Implicite_BCP()
{
	...
	StopWatch tmp;
	double tim;
	old_entropy_num_levels = _num_levels;
	running_options.erase_useless_cacheable_component = false;
...
//			Display_Comp_And_Decision_Stacks( cerr );  // ToRemove
		...
		if ( Num_Components_On_Current_Level() <= 1 ) { // decision or preparation 	逐一尝试 false true
			...
			case 0: // 蕴含文字
				...
				assert(_active_comps[_num_levels - 2] <= _active_comps[_num_levels - 1]); 
				tmp.Start();
			...
			if ( Is_Current_Level_Active() ) {  // not all components have been processed
				...
				case 1:
					if ( _rsl_stack[_num_rsl_stack - 1] != 0 ) {
						...
						dynamic_time += tmp.Get_Elapsed_Seconds();
...
void KCounter::Backjump_Decision( unsigned num_kept_levels )
{
	...
	num_kept_levels = max(num_kept_levels,old_entropy_num_levels); // thl add
...
void KCounter::Component_Cache_Erase( Component & comp )
{
	...
	for ( unsigned i = old_entropy_num_levels - 1; i < _num_levels; i++ ) { // pre code : unsigned i = 1; i < _num_levels; i++ 
...
void KCounter::Backtrack_Known( BigInt cached_result )
{
	...
	//cout<<"pre = "<<_rsl_stack[_num_rsl_stack]<<endl;
	_rsl_stack[_num_rsl_stack] = cached_result;
	//cout<<"after="<<_rsl_stack[_num_rsl_stack]<<endl;
...
void KCounter::Component_Cache_Clear()
{
	...
	for ( unsigned i = old_entropy_num_levels - 1 ; i < _num_levels; i++ ) { // update pre is i = 1 ,now is i = old_num_levels - 1
...
{
	_num_levels--;
	//cout<<"Un_BCP to "<<_dec_offsets[_num_levels]<<endl; // toremove
	Un_BCP( _dec_offsets[_num_levels] ); // precode
	//cout <<" alright now the ~level = "<<_dec_offsets[_num_levels]<<endl;
	_num_comp_stack = _comp_offsets[_num_levels];
}
...
//	cerr << _rsl_stack[_num_rsl_stack - 2] << " vs " << _rsl_stack[_num_rsl_stack - 1] << endl;  // ToRemove
	...
	//cout<<"omit = "<<omit<<" Current_Component().Vars_Size() = "<<Current_Component().Vars_Size()<<"  _aux_rsl_stack[_num_rsl_stack - 1] = "<<_aux_rsl_stack[_num_rsl_stack - 1]<<endl;
	unsigned varsize = Current_Component().Vars_Size();
	_rsl_stack[_num_rsl_stack - 1].Mul_2exp( omit );
	//cout<<"omit = "<<omit<<" num_stack -1 = "<<_num_rsl_stack-1<<"  res["<<_num_rsl_stack-1<<"]= "<<_rsl_stack[_num_rsl_stack-1]<<endl;
	omit = Current_Component().Vars_Size() - _aux_rsl_stack[_num_rsl_stack] - 1;
	//cout<<"pre    omit="<<omit<<"  num_stack = "<<_num_rsl_stack<<"  res["<<_num_rsl_stack<<"]="<<_rsl_stack[_num_rsl_stack]<<endl;
	_rsl_stack[_num_rsl_stack].Mul_2exp( omit );
	//cout<<"after  omit="<<omit<<"  num_stack = "<<_num_rsl_stack<<"  res["<<_num_rsl_stack<<"]="<<_rsl_stack[_num_rsl_stack]<<endl;
	_rsl_stack[_num_rsl_stack - 1] += _rsl_stack[_num_rsl_stack];
	//cout<<"num_stack -1 = "<<_num_rsl_stack - 1<<"  res["<<_num_rsl_stack-1<<"]= "<<_rsl_stack[_num_rsl_stack - 1]<<endl;
	...
	//cout<<" _aux_counting_stack["<<_num_rsl_stack - 1<<"] = "<<_aux_rsl_stack[_num_rsl_stack - 1]<<endl;
	if ( debug_options.verify_component_count ) {
...
void KCounter::Backtrack_Decomposition()
{
	...
	for ( unsigned i = 2; i < Num_Components_On_Current_Level(); i++ ) {
		...
		//cout<<"_aux_counting_stack["<<_num_rsl_stack + i<<"] = "<<_aux_rsl_stack[_num_rsl_stack + i]<<endl;
	}
	//cout<<" Backtrack_Decomposition rsl = "<<_rsl_stack[_num_rsl_stack]<<endl; // thl to remove
	_aux_rsl_stack[_num_rsl_stack] += Num_Current_Imps();
	//cout<<"Num_Current_Imps() = "<<Num_Current_Imps()<<" _aux_counting_stack["<<_num_rsl_stack<<"]="<<_aux_rsl_stack[_num_rsl_stack]<<endl;
	_num_rsl_stack++;
	Backtrack();
}
...
lbool KCounter::Try_Final_Kernelization()
{
	...
	BigInt cached_result;
	unsigned old_num_levels = Get_old_num_levels();
