...
public:
	...
	vector<unsigned>::iterator VarIDs_begin() { return _varIDs.begin(); } // thl add
	vector<unsigned>::iterator VarIDs_end() { return _varIDs.end(); } // thl add 

	vector<unsigned>::iterator ClauseIDs_begin() { return _clauseIDs.begin(); } // thl add
	vector<unsigned>::iterator ClauseIDs_end() { return _clauseIDs.end(); }  // thl add
	vector<unsigned>::iterator Erase_Var( vector<unsigned>::iterator st ) { // thl 删除变量
		return _varIDs.erase(st);
	}
	vector<unsigned>::iterator Erase_Clause( vector<unsigned>::iterator st ) { // thl 删除变量
		return _clauseIDs.erase(st);
	}
	...
	{
		if (_varIDs.size() < 1) return; // thl to fix
	...
	void Display_Debug()
	{
		...
		unsigned i;
		cout << "varIDs: ";
		for ( i = 0; i < _varIDs.size(); i++ ) cout << _varIDs[i] << " ";
		cout << endl;
		cout << "clauseIDs: ";
		for ( i = 0; i < _clauseIDs.size(); i++ ) cout << _clauseIDs[i] << " ";
		cout << endl;
	}
};
...
public:
	...
	void Clear_Half( vector<size_t> & kept_locs ) // thl add
	{
		vector<bool> seen( _pool.Size(), false );
		for ( unsigned i = 0; i < kept_locs.size(); i++ ) {
			seen[kept_locs[i]] = true;
