...
{
protected:
	int * visited; // thl used for shortest path 
	int ** shortest; // thl compute the shortest distance between any two vertice used for Centrality score
	long long ** path_count; // thl Calculate the number of schemes for the shortest distance between any two vertice
	vector<unsigned> * vec; // thl primal graph
	double * centr_score; // used for centrality score
	
	double * Treed_Score; // used for Treed score
	int * dist_cluster;
	...
	Chain var_order_X; // X 部分树分解的minfill thl
	...
	Component * Allvar_Compstack;  // 记录Y决策过程中所有变量组成的组件分量 recording all components on the path Y
	Component * Ylist_Compstack; // 记录Y决策过程中仅包含Y变量的组件分量
	
	Component * _comp_stack;  // recording all components on the path
	
	unsigned _num_all_stack;
	unsigned _num_comp_stack;
	unsigned * comp_id; // thl Y搜索中 comp_id[i] 记录决策等级为i的是哪个组件（组件下标）
...
protected:
	void Compute_Treed_Score(unsigned root,vector<Simple_TreeD_Cluster> _clusters); // thl add
...
protected:  /// Var Heuristic
	Variable Pick_Good_Var_Centrality( Component & comp ); // thl heurstic
	Variable Pick_Good_Var_Treetd( Component & comp ); // thl heurstic
	void Compute_Var_Centrality_Score(); // thl to comput centrality score
	void Centrality_allocate(); // thl 中心分数分配空间
	bool Centrality_flag = false;
	...
	Variable Pick_Good_VarX_Linearly_Component( Component & comp );// thl
...
protected:  /// compute implied literals by IBCP
	...
	Reason Get_Approx_Imp(unsigned & backjump_level ); // thl 隐式BCP
	Reason Get_Approx_Imp_Component_Full_IBCP( Component & comp );
	Reason Get_Approx_Imp_Full_IBCP( ); // thl 计算蕴含文字
	...
	Reason Get_Approx_Imp_Partial_IBCP(); // thl 部分IBCP
...
protected:  /// manage components
	void Generate_Init_YComponent( Component & comp );  // thl
	void Generate_Init_XYComponent( Component & comp );  // thl
	...
	Component & Parent_of_Current_Component() { 
		return _comp_stack[_active_comps[_num_levels - 2]]; 
	}
	Component & Current_Ylist_Component() { // thl
		return Ylist_Compstack[comp_id[_num_levels - 1]]; 
	}
	Component & Parent_of_Ylist_Component() {  // thl
		return Ylist_Compstack[comp_id[_num_levels - 2]]; 
	}
	...
	void GetYlist_NextComponent( Component & source , Component & comp );
	void GetAllvar_NextComponent( Component & source , Component & comp );
...
protected:  /// process models
	...
	void Debug_Display_Models( vector<Model *> & source);
protected:
