...
void Greedy_Graph::Caculate_Shortest_distances(int ** shortest, long long ** path_count, int * visited,vector<unsigned> * vec) { // thl 计算最短路
	queue<int>q;
	for (int u = 1 ; u <= max_vertex ; u ++) {
		if ( vertices[u].degree == UNSIGNED_UNDEF || visited[u] == -1) continue;
	...
	for (int i = 0 ; i <= max_vertex ; i ++) {
		if ( vertices[i].degree == UNSIGNED_UNDEF || visited[i] == -1) continue;
		while (!q.empty()) {
			q.pop();
		}
		q.push(i);
		...
		while(!q.empty()) {
			...
			while ( p->vertex <= max_vertex ) {
				int v = p->vertex;
				if (!visited[v]) {
					shortest[i][v] = shortest[i][u] + 1;
					path_count[i][v] = path_count[i][u];
					visited[v] = 1;
					q.push(v);
				}
				else {
					if (shortest[i][u] + 1 == shortest[i][v]) path_count[i][v] += path_count[i][u];
				}
				p = p->next;
			}
		}
	...
	// 	if ( vertices[u].degree == UNSIGNED_UNDEF ) continue;
...
void Greedy_Graph::Debug_Display( ) // thl debug
{
	cout << "Vertices:";
	...
	if ( last != UNSIGNED_UNDEF ) {
		for ( unsigned v = 0; v < last; v++ ) {
			if ( vertices[v].degree != UNSIGNED_UNDEF ) cout << " " << v << ',';
		}
		cout << " " << last;
	}
	cout << endl
	...
	for ( unsigned v = 0; v <= max_vertex; v++ ) {
		...
		if ( vertices[v].adjacency->next != common_tail_neighbour ) cout << char( 8 ) << ';';
		cout << endl;
	}
	cout << "End." << endl;
}
void Greedy_Graph::Display( ostream & out )
{
	...
	if ( last != UNSIGNED_UNDEF ) {
		for ( unsigned v = 0; v < last; v++ ) {
			if ( vertices[v].degree != UNSIGNED_UNDEF ) out << " " << v << ',' << vertices[v].infor << endl;
		}
		out << " " << last;
	}
	out << endl
...
Simple_TreeD::Simple_TreeD( Greedy_Graph & graph, unsigned bound, vector<int> & belong_XORY, bool opt ): _max_vertex( graph.Max_Vertex() ), _clusters( 2 )  /// use SList optimize it
{
	assert( opt == true );  /// use SList optimize it
	Allocate_and_Init_Auxiliary_Memory();
	unsigned min_fill_var;
	...
	//graph.Display( cout ); // vertex 的 infor 记录的是每个节点的minfill 即删除他之后，他的所有邻居要构成团需要加多少条边
	vector<Simple_TreeD_Cluster> tmp_clusters;
	SList<unsigned> left_vertices;
	unsigned num_left_vertices = 0;
	SList_Node<unsigned> * pre = left_vertices.Head();
...
//		graph.Validate_Fill_Size();
		SList_Node<unsigned> * min_fill_pre = left_vertices.Head();
		min_fill_var = min_fill_pre->next->data;
		bool find_Y = false; // thl 让树分解优先选择Y变量
		for ( pre = min_fill_pre->next; pre->next != nullptr; pre = pre->next ) {
			unsigned var = pre->next->data;
			//if (!belong_XORY[var]) continue; // thl 
			find_Y = true;
			if ( graph.vertices[var].infor < graph.vertices[min_fill_var].infor ) {
				min_fill_pre = pre;
				min_fill_var = var;
		...
		left_vertices.Delete_After( min_fill_pre );
		//cout << min_fill_var << ": " << graph.vertices[min_fill_var].degree << " vs "<< graph.vertices[min_fill_var].infor << " " << belong_XORY[min_fill_var] <<endl;
		if ( graph.vertices[min_fill_var].degree > bound ) {  /// exceed bound
			Generate_Singleton_Cluster();
			for ( i = tmp_clusters.size() - 1; i != UNSIGNED_UNDEF; i-- ) {
				delete [] tmp_clusters[i].vars;
		...
		cluster.infor = cluster.num_vars;  /// record the position of min_fill_var
		// cluster的infor记录的是该点的邻居数量
		//cout<<"cluster.infor = "<<cluster.infor<<endl;
		cluster.vars[cluster.num_vars++] = min_fill_var;
		for ( ; p->vertex <= graph.max_vertex; p = p->next ) {
			cluster.vars[cluster.num_vars++] = p->vertex;
		}
...
//		Display( cout, cnf );
	...
	treed_root =  Compute_Root_Lowest_Weighted_Depth_Reserved();
}
