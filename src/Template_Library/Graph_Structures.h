...
public:
	...
	bool Less_Than( unsigned elem1, unsigned elem2 ) const
	{
		...
		//cout<<"elem1 = "<<elem1<<" rank1 = "<<_rank[elem1]<<" elem2 = "<<elem2<<" rank2 = "<<_rank[elem2]<<endl;
...
public:
	...
	void Caculate_Shortest_distances(int ** shortest, long long ** path_count, int * visited, vector<unsigned> * vec); // thl to caculate shortest distance 
...
public:
	unsigned treed_root = 99999999; // thl add
...
public:
	...
	Simple_TreeD( Greedy_Graph & graph, unsigned bound, vector<int> & belong_XORY,bool opt );  /// using min-fill to generate tree decomposition from graph, and if the treewidth exceeds bound, then terminate
