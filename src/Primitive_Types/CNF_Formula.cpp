...
void CNF_Formula::Read_XYlist( char line[] ) { // thl
	assert( line[0] == 'c' );
	char *p = line + 1;
	int tag = 0; // 0 represent  x list 1 represent y list
	unsigned old_x_size = Xlist.size();
	unsigned old_y_size = Ylist.size();
	//int tmp = _max_var + 1;
	if ( Read_String_Change( p, "max" ) ) tag = 0; 
	else if ( Read_String_Change( p, "ind" ) ) tag = 1;
	while ( *p == ' ' || *p == '\t' ) p++;
	while ( 1 ) {
		int tmp = -1;
		if ( sscanf( p, "%d", &tmp) != 1 ) {
		...
		if ( tmp == 0 ) break;
		if (!tag) Xlist.push_back( tmp );
		else Ylist.push_back(tmp);
		//cout<<" tmp = "<<tmp<<endl;
		while ( '0' <= *p && *p <= '9' ) p++;
		while ( *p == ' ' || *p == '\t' ) p++;
	}
...
bool WCNF_Formula::Get_Line_MiniC2D( istream & fin, char line[] )
{
	while ( !fin.eof() ) {
		...
		if ( line[0] == 'c' ) {
			char * p = line + 1;
			// thl if ( !Read_String_Change( p, "weights") ) continue;
			if ( !_weights.empty() ) {
