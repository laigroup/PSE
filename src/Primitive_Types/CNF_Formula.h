...
class CNF_Formula
{
	vector<unsigned> Xlist; // thl
	vector<unsigned> Ylist; // thl 
...
public:
	...
	vector<unsigned> & Get_Xlist() { return Xlist; } // thl
	vector<unsigned> & Get_Ylist() { return Ylist; } // thl
	...
	void Copy_Ylist(vector<unsigned> & other) { Ylist = other; }
	void Clear_Ylist() { Ylist.clear(); }
	void Sort_Clauses();
...
protected:
	...
	void Read_XYlist( char line[] ); // thl
