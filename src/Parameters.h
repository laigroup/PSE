...
struct PSE: public Tool_Parameters
{
	StringOption ConjunctiveDecomposition;
	StringOption Pre;
	StringOption XCache;
	StringOption YCache;
	StringOption Counting;
	StringOption heur;
	PSE( const char * tool_name ): Tool_Parameters( tool_name ),
		ConjunctiveDecomposition( "--ConjunctiveDecomposition", "Conjunctive Decomposition for PSE (false, true)", "true" ),
		Pre( "--Pre", "preprocess for PSE (false, true)", "true" ),
		XCache( "--XCache", "XCache in X-stage (false, true)", "true" ),
		YCache( "--YCache", "YCache in Y-stage (false,true)", "true" ), 
		Counting( "--Counting", "counting method in X-stage (SharedCounting, ConditionedCounting)", "SharedCounting" ),
		heur( "--heur", "heuristic strategy (auto, minfill, SharpSAT_TD, VSADS, DLCS, DLCP)", "minfill" )
	{
		Add_Option( &ConjunctiveDecomposition );
		Add_Option( &Pre );
		Add_Option( &XCache );
		Add_Option( &YCache );
		Add_Option( &Counting );	
	...
	bool Parse_Parameters( int & i, int argc, const char *argv[] )
	{
		...
		if (strcmp(Counting,"SharedCounting") != 0 && strcmp(Counting,"ConditionedCounting") != 0) return false;
		if ( strcmp( heur, "auto") != 0 && strcmp( heur, "minfill") != 0 && \
			strcmp( heur, "SharpSAT_TD") != 0 && strcmp( heur, "VSADS") != 0 && strcmp( heur, "DLCS") != 0 && strcmp( heur, "DLCP") != 0) {
			return false;
		}
		if (strcmp(ConjunctiveDecomposition,"true") != 0 && strcmp(ConjunctiveDecomposition,"false") != 0) return false;
		if (strcmp(Pre,"true") != 0 && strcmp(Pre,"false") != 0) return false;
		if (strcmp(YCache,"true") != 0 && strcmp(YCache,"false") != 0) return false;
		if (strcmp(XCache,"true") != 0 && strcmp(XCache,"false") != 0) return false;
		return true;
	}
};
...
extern inline Heuristic Parse_Heuristic( const char * heur )
{
	...
	else if ( strcmp( heur, "SharpSAT_TD" ) == 0 ) return SharpSAT_TD;
	else {
...
/// parameters of solver
	...
	double Time_limit; // thl add 整体运行的时间限制
...
/// parameters of oracle
	...
	Running_Options()
	{
		ConjunctiveDecomposition = true;
		XCache = true;
		YCache = true;
		Pre = true;
		Counting = "SharedCounting";


		Time_limit = 30000; // thl add 时间限制
		...
		display_preprocessing_process = false;
