...
Reason Inprocessor::Get_Approx_Imp_Full_IBCP( )
{
	// Reason confl = BCP( _num_dec_stack - 1 );  // NOTE: cannot replace with _dec_offsets[_num_levels - 1] because of backjump
...
Reason Inprocessor::Get_Approx_Imp_Partial_IBCP() // thl
{
	unsigned i, j, k;
	Reason confl = BCP(_num_dec_stack - 1 );  // NOTE: cannot replace with _dec_offsets[_num_levels - 1] because of backjump
 	if ( confl != Reason::undef ) return confl;
	...
	while ( old_num_d_stack < _num_dec_stack ) {
		...
		for ( i = 0; i < num_active_lits; i++ ) {
			...
			Assign( lit );
			confl = BCP(_dec_offsets[_num_levels - 1] );
			if ( confl != Reason::undef ) {
				Analyze_Conflict_Fixed_UIP( confl, lit );
				Un_BCP( _dec_offsets[_num_levels - 1] );
				_num_levels--;
				Assign( ~lit, Add_Learnt() );
				confl = BCP(_num_dec_stack - 1 );  // NOTE: cannot use _dec_offsets[_num_levels - 1]
				if ( confl != Reason::undef ) return confl;
...
Reason Inprocessor::Get_Approx_Imp( unsigned & backjump_level ) // thl
{
	StopWatch tmp_watch;
	...
	case Full_Implicit_BCP:
		confl = Get_Approx_Imp_Full_IBCP();
		assert(0);
		break;
	case Partial_Implicit_BCP:
		confl = Get_Approx_Imp_Partial_IBCP();
		break;
	default:
...
void Inprocessor::GetYlist_NextComponent( Component & source , Component & comp )
{
	StopWatch begin_watch;
	if ( running_options.profiling_inprocessing >= Profiling_Detail ) begin_watch.Start();
	comp = source;
	vector<unsigned>::iterator st = comp.VarIDs_begin();
	vector<unsigned>::iterator ed = comp.VarIDs_end();
	while(st != ed) {
		Variable var = Variable(*st);
		if (Var_Decided(var)) {
			st = comp.Erase_Var(st);
		}
		else st ++;
		ed = comp.VarIDs_end();
	}
	// st = comp.ClauseIDs_begin();
	// ed = comp.ClauseIDs_end();
	// while(st != ed) {
	// 	unsigned id = *st;
	// 	if (Clause_SAT(_long_clauses[id])){
	// 		st = comp.Erase_Clause(st);
	// 	}
	// 	else st ++;
	// 	ed = comp.ClauseIDs_end();
	// }
}
void Inprocessor::GetAllvar_NextComponent( Component & source , Component & comp )
{
	StopWatch begin_watch;
	if ( running_options.profiling_inprocessing >= Profiling_Detail ) begin_watch.Start();
	comp = source;
...
void Inprocessor::Allocate_and_Init_Auxiliary_Memory( Variable max_var )  // ToDo: whether can we optimize when max_var < _max_var
{
	...
	Allvar_Compstack = new Component [2 * max_var + 2]; // thl 
	Ylist_Compstack = new Component [2 * max_var + 2]; // thl 
	comp_id =  new unsigned [2 * max_var + 2]; // thl
...
{
	delete [] Allvar_Compstack; 
	delete [] Ylist_Compstack;
	delete [] comp_id;
...
void Inprocessor::Centrality_allocate() // thl to compute centrality score
{
	shortest = new int * [_max_var + 5];
	path_count = new long long * [_max_var + 5];
	visited = new int [_max_var + 5];
	vec = new vector<unsigned> [_max_var + 5];
	centr_score = new double [_max_var + 5];
	for (int i = 0 ; i <= _max_var ; i ++) {
		shortest[i] = new int [_max_var + 5];
		path_count[i] = new long long [_max_var + 5];
	}
}
void Inprocessor::Compute_Var_Centrality_Score() // thl to compute centrality score
{
	StopWatch begin_watch;
	begin_watch.Start();
	Greedy_Graph * pg = Create_Primal_Graph();
	//pg->Debug_Display(); // thl debug toremove
	if (Centrality_flag == false) {
		Centrality_flag = true;
		Centrality_allocate();
	}
	for (int i = 0 ; i <= _max_var ; i ++) {
		...
		for (int j = 0 ; j <= _max_var ; j ++) {
			shortest[i][j] = _max_var * 10;
			path_count[i][j] = 0;
		}
	}
	pg->Caculate_Shortest_distances(shortest,path_count,visited,vec);
	delete pg;
	// for (int u = 1 ; u <= _max_var ; u ++) {
	// 	for (int v = 1 ; v <= _max_var ; v ++) {
	// 		cout<<" u = "<<u<<" v = "<<v<<" shortest = "<<shortest[u][v]<<" path number = "<<path_count[u][v]<<endl;
	// 	}
	// }
	// cout<<" edge : "<<endl;
	// for (int u = 1 ; u <= _max_var ; u ++) {
	...
	// 	cout<<endl;
	// }
	for (int k = 1 ; k <= _max_var ; k ++) {
		Variable var;
		var = Variable(k);
		centr_score[k] = 0;
		if (Var_Decided(var)) continue;
		//if (belong_XORY[k]) continue;
		for (int u = 1 ; u <= _max_var ; u ++) {
			if (shortest[u][k] == _max_var * 10 || u == k) continue;
			var = Variable(u);
			if (Var_Decided(var)) continue;
			//if (belong_XORY[u]) continue;
			for (int v = u + 1 ; v <= _max_var ; v ++) {
				if (shortest[k][v] == _max_var * 10 || u == v || v == k) continue;
				var = Variable(v);
				//if (belong_XORY[v]) continue;
				if (Var_Decided(var)) continue;
				int factor = 1;
				//if (!belong_XORY[u] && !belong_XORY[v] && belong_XORY[k]) factor = 10;
				if (belong_XORY[u]) factor *= 5;
				if (belong_XORY[v]) factor *= 5;
				if (shortest[u][k] + shortest[k][v] == shortest[u][v]) {
					double val = 1.0 * path_count[u][k] * path_count[k][v];
					centr_score[k] += val / path_count[u][v] * factor;
				}
			}
		}
	}

	//statistics.time_tree_decomposition = begin_watch.Get_Elapsed_Seconds();
}
void Inprocessor::Compute_Treed_Score(unsigned root,vector<Simple_TreeD_Cluster> _clusters) {
	queue<unsigned>q;
	q.push(root);
	for (int i = 0 ; i <= _max_var ; i ++) {
		Treed_Score[i] = 1e9;
	...
	//cout<<" root= "<<root<<" _clusters.size() = "<<_clusters.size()<<endl;
	dist_cluster[root] = 0;
	while(!q.empty()) {
		unsigned u = q.front();
		//cout<<" u = "<<u<<endl;
		q.pop();
		for ( int j = 0; j < _clusters[u].num_vars; j++ ) {
			unsigned var = _clusters[u].vars[j];
			Treed_Score[var] = min(Treed_Score[var],dist_cluster[u] * 1.0);
		}
		TreeD_Cluster_Adjacency * p = _clusters[u].adj->next;
		while ( p->ordinal < cluster_size ) {
			//cout<<" p = "<<p->ordinal<<endl;
			if (dist_cluster[p->ordinal] == -1) {
				dist_cluster[p->ordinal] = dist_cluster[u] + 1;
				q.push(p->ordinal);
			}
			p = p->next;
		}
	}
	double minn = 1e9 + 7 ,maxx = -1;
	for (int i = Variable::start ; i <= _max_var ; i ++) { 
		if (Treed_Score[i] > cluster_size) continue;
		minn = min(minn,Treed_Score[i]);
		maxx = max(maxx,Treed_Score[i]);
	}
	for (int i = Variable::start ; i <= _max_var ; i ++) {
		if (Treed_Score[i] > cluster_size) continue;
		Treed_Score[i] = (Treed_Score[i] - minn) / (maxx - minn);
		if (Treed_Score[i] > 1) printf("%.2f\n",Treed_Score[i]);
		assert(Treed_Score[i] <= 1);
	}
	// for (int i = 0 ; i < cluster_size ; i ++) {
	// 	cout<<" i = "<<i<<" dist = "<<dist_cluster[i]<<endl;
	// }
	// for (int i = 1 ; i <= _max_var ; i ++) {
	// 	cout<<" var = "<<i<<" treed score = "<<Treed_Score[i]<<endl;
	// }
}
void Inprocessor::Compute_Var_Order_Min_Fill_Heuristic_Bound( unsigned bound )
{
	...
	if ( _unsimplifiable_num_vars == _max_var ) {
		...
		Simple_TreeD treed( *pg, bound, belong_XORY ,true );  /// optimized for large-scale problems
		running_options.treewidth = treed.Width();
		
		
		unsigned root = treed.treed_root;
		vector<Simple_TreeD_Cluster> _clusters = treed._clusters;
		Treed_Score = new double [_max_var + 5];
		dist_cluster = new int [_clusters.size() + 5];
		if (root != 99999999) Compute_Treed_Score(root,_clusters);
		 // 暂时不用自定义的启发式
		 
		if ( running_options.treewidth <= bound ) {
			Generate_Var_Order_From_TreeD( treed );
		}
		// cout<<"treewidth = "<<running_options.treewidth<<" bound = "<<bound<<endl;
		// Generate_Var_Order_From_TreeD( treed );
	}
	delete pg;
...
void Inprocessor::Compute_Var_Order_Min_Fill_Heuristic_Opt()
{
	...
	running_options.treewidth = treed.Width();
	//cout<<"minfill tree width = "<<running_options.treewidth<<endl;
	Generate_Var_Order_From_TreeD( treed );
	delete pg;
	if ( running_options.profiling_inprocessing >= Profiling_Detail ) statistics.time_tree_decomposition = begin_watch.Get_Elapsed_Seconds();
}
...
void Inprocessor::Var_Weight_For_Tree_Decomposition_Without_Sorting( double * var_weight )
{
	...
	for ( i = Variable::start; i <= _max_var; i++ ) {
		...
		case 2:
			factor = 1; // thl 
			scores[0] = _old_num_binary_clauses[i + i] + _binary_clauses[i + i].size(); // precode
			scores[1] = _old_num_binary_clauses[i + i + 1] + _binary_clauses[i + i + 1].size(); //precode
			// scores[0] = _old_num_binary_clauses[i + i];
			// scores[1] = _old_num_binary_clauses[i + i + 1];
			// for (int it = 0 ; it <_binary_clauses[i + i].size() ; it ++) {
			// 	Literal lit = _binary_clauses[i + i][it];
			// 	if (lit.Var() == i) continue;
			// 	if (belong_XORY[lit.Var()]) scores[0] += factor * 2;
			// 	else scores[0] += factor; 
			// }
			// for (int it = 0 ; it <_binary_clauses[i + i + 1].size() ; it ++) {
			// 	Literal lit = _binary_clauses[i + i + 1][it];
			// 	if (lit.Var() == i) continue;
			// 	if (belong_XORY[lit.Var()]) scores[1] += factor * 2;
			// 	else scores[1] += factor;  
			// }
			itr = _ternary_var_membership_lists[i].begin(), end = _ternary_var_membership_lists[i].end();
			factor = 1; // thl 
			for ( ; itr < end; itr += 3 ) {
				Clause & clause = _long_clauses[*itr];
				for ( j = 0; j < clause.Size() ; j++ ) { // thl
					if (clause[j].Var() == i) continue;
					Literal lit = clause[j];
					if (belong_XORY[lit.Var()]) factor *= 2;
					else factor ++;
				} 
				for ( j = 0; clause[j].Var() != i; j++ ) {}
				scores[clause[j].Sign()] += 1.0 / 3; // pre code
				//scores[clause[j].Sign()] += 1.0 / 3 * factor; // thl
			}
			factor = 1;
			...
			for ( ; itr < end; itr += 4 ) {
				...
				for ( j = 0; clause[j].Var() != i; j++ ) {}
				scores[clause[j].Sign()] += 1.0 / 4; // precode
				//scores[clause[j].Sign()] += 1.0 / 4 * factor; 
			}
			factor = 1;
			itr = _long_var_membership_lists[i].begin(), end = _long_var_membership_lists[i].end();
			for ( ; itr < end; itr++ ) {
				...
				for ( j = 0; clause[j].Var() != i; j++ ) {}
				scores[clause[j].Sign()] += 1.0 / clause.Size(); // precode
				//scores[clause[j].Sign()] += 1.0 / clause.Size() * factor;
			}
			var_weight[i] = scores[0] * scores[1];
			break;
		default:
...
void Inprocessor::Generate_Var_Order_From_TreeD( Simple_TreeD & treed )
{
	...
	for ( unsigned i = 0; i < init_order.Size(); i++ ) { // thl add
		Variable var( init_order[i] );
		if (!belong_XORY[ init_order[i] ]) continue;
		if ( Var_Appeared( var ) ) {
			_var_order.Append( var );
		}
	}
	for ( Variable i = Variable::start; i <= _max_var; i++ ) { // thl add
		if (!belong_XORY[ init_order[i] ]) continue;
...
unsigned Inprocessor::Compute_Var_Order_Min_Fill_Bound_Component( Component & comp, unsigned bound )
{
	...
	if ( treewidth <= bound ) {
		var_order_X.Clear(); // now code thl
		assert( var_order_X.Empty() ); // pre code
...
void Inprocessor::Generate_Var_Order_Min_Fill_Component( Component & comp, Simple_TreeD & treed, double * var_weight )
{
	...
	for ( unsigned i = 0; i < init_order.Size(); i++ ) {
		...
		ASSERT( !var_order_X.Contain( var ) );
		var_order_X.Append( var );
	}
}
...
void Inprocessor::Generate_Init_YComponent( Component & comp ) // thl
{
	StopWatch tmp_watch;
	...
	Variable i ; 
	for ( i = Variable::start; i <= _max_var; i++ ) { 
		if (belong_XORY[i] == 1 && Var_Undecided(i) && Var_Appeared(i))  comp.Add_Var( i );
	} 
	...
	for ( i = 0; i < _old_num_long_clauses; i++ ) {
		Clause cl = _long_clauses[i];	
		bool flag = false;
		for (int j = 0 ; j < cl.Size() ; j ++) {
			Literal lit = cl[j];
			if (belong_XORY[lit] == 1) {
				flag = true;
				break;
			}	
		}
		if (flag == true) comp.Add_ClauseID( i );
	}
	//cout<<" NOW THE INIT VAR SIZE = "<<comp.Vars_Size()<<endl;
}

void Inprocessor::Generate_Init_XYComponent( Component & comp )
{
	StopWatch tmp_watch;
	if ( running_options.profiling_inprocessing >= Profiling_Detail ) tmp_watch.Start();
	comp.Clear();  // NOTE: comp is initialized here
	Variable i ; // pre code is Variable i;
	for ( i = Variable::start; i <= _max_var; i++ ) {  /// NOTE: must be sorted with respect to the lexicographic order for binary search
		if (Var_Undecided(i) && Var_Appeared(i)) comp.Add_Var( i );
	} // pre code is i ++; 
	comp.Add_Var( _max_var.Next() );
	comp.Dec_Var();  /// NOTE: prevent comp.Vars() from reallocating memory when push_back mar_var + 1 later

	// if (running_options.ImpliedLiteral == false) { // 不使用蕴含文字的时候 存储组件的时候要把二元子句存储进去
	...
	// 			Clause tmp_cls = Clause( lit, _binary_clauses[lit][i] );
	// 			_long_clauses.push_back(tmp_cls);
	// 			//tmp_cls.Display(cout);
	// 		}
	// 	}
...
unsigned Inprocessor::Dynamic_Decompose_Component_Without_Sorting( Component & source, Component smaller_comps[] )
{
	StopWatch begin_watch;
	if ( running_options.profiling_inprocessing >= Profiling_Detail ) begin_watch.Start();
	unsigned num_vars, num_cls; // num_vars 记录组件source还有多少个未决策变量 num_cls记录有多少个还不是SAT的子句
	...
	// cout<<" smaller cp is "<<endl;
	// smaller_comps->Display_Debug();
	...
	while ( num_vars > 0 ) {
		...
		// cout<<"before size is "<<comp->Vars_Size()<<endl;
		comp->Vars_Resize( 1 );
		// cout<<"after size is "<<comp->Vars_Size()<<endl;
		unsigned num_rest;
		assert(num_vars > 0);
...
{
	switch ( running_options.var_ordering_heur ) {
	case SharpSAT_TD: // sharpSAT-TD heur
		return Pick_Good_Var_Treetd( comp );
		break;
	case Centrality:
		return Pick_Good_Var_Centrality( comp ); // thl new heursitic
		break;
	case minfill:
...
Variable Inprocessor::Pick_Good_Var_Treetd( Component & comp )  // thl heurstic
{
	const double factor = 10 / running_options.sat_heur_cumulative_inc;
	const double tree_w = running_options.treewidth;
	const double treed_factor = exp(_max_var / tree_w) / _max_var;; //exp(_max_var / tree_w) / _max_var; // thl 
	//printf("facotr = %.2f\n",treed_factor);
	vector<unsigned>::const_iterator itr, end;
	for ( itr = comp.VarIDs_Begin(), end = comp.VarIDs_End(); itr < end; itr++ ) {
		...
		//_var_scores[var] = centr_score[var] * centra_factor;
		for ( unsigned i = 0; i < _binary_var_membership_lists[var].size(); i++ ) {
			Variable vari = _binary_var_membership_lists[var][i];
			double fct;
			if (belong_XORY[vari] == 1) fct = 10;
			else fct = 1;
			_var_scores[var] += Var_Undecided( _binary_var_membership_lists[var][i] ); // pre code
			//_var_scores[var] += Var_Undecided( _binary_var_membership_lists[var][i] ) * factor;
		}
	}
	...
	_var_scores[*itr] -= treed_factor * Treed_Score[*itr];
	for ( itr++, end = comp.VarIDs_End(); itr < end; itr++ ) {
		if (!belong_XORY[*itr]) continue;
		find_Y = true;
		_var_scores[*itr] -= treed_factor * Treed_Score[*itr];
		if ( _var_scores[*itr] > _var_scores[*best] || !belong_XORY[*best]) {
			best = itr;
		}
	}
	if (!find_Y) 
...
Variable Inprocessor::Pick_Good_Var_Centrality( Component & comp )  // thl heurstic
{
	const double factor = 10 / running_options.sat_heur_cumulative_inc;
	const double centra_factor = 10; // thl 
	vector<unsigned>::const_iterator itr, end;
	for ( itr = comp.VarIDs_Begin(), end = comp.VarIDs_End(); itr < end; itr++ ) {
		unsigned var = *itr;
		ASSERT( !Var_Decided( Variable( var ) ) );
		double sum = _heur_decaying_sum[var + var] + _heur_decaying_sum[var + var + 1];
		_var_scores[var] = sum * factor + centr_score[var] * centra_factor;
		//_var_scores[var] = centr_score[var] * centra_factor;
	...
	vector<unsigned>::const_iterator best = itr;
	for ( itr++, end = comp.VarIDs_End(); itr < end; itr++ ) {
		//cout<<"var = "<<(*itr)<<" scores = "<<_var_scores[*itr]<<endl; 
		if ( _var_scores[*itr] > _var_scores[*best] ) {
			best = itr;
		}
	}
...
Variable Inprocessor::Pick_Good_VarX_Linearly_Component( Component & comp ) // thl
{
	vector<unsigned>::const_iterator itr, begin, end, best;
	itr = comp.VarIDs_Begin();
	best = itr;
	for ( itr++, end = comp.VarIDs_End(); itr < end; itr++ ) {
		assert( !Var_Decided( Variable( *itr ) ) ); //pre code
		if (Var_Decided( Variable( *itr ))) continue; // now code
		if (Var_Decided( Variable( *best ) )) best = itr;
		if ( var_order_X.Less_Than( *itr, *best ) ) {
			best = itr;
		}
	}
	return Variable( *best );
...
Variable Inprocessor::Pick_Good_Var_Linearly_Component( Component & comp )
{
	...
	best = itr;
	for ( itr++, end = comp.VarIDs_End(); itr < end; itr++ ) {
		//if (Var_Decided( Variable( *itr ))) cout<<" decide var = "<<(*itr)<<endl;
		assert( !Var_Decided( Variable( *itr ) ) ); //pre code
		//if (Var_Decided( Variable( *itr ))) continue; // now code
		if (Var_Decided( Variable( *best ) )) best = itr;
...
double Inprocessor::Var_Score_VSADS_Without_Sorting( unsigned var )  // using _clause_status
{
	...
	for ( unsigned i = 0; i < _ternary_var_membership_lists[var].size(); i += 3 ) {
		cout<<"value of _ternary_var_membership_lists is "<<_clause_status[_ternary_var_membership_lists[var][i]]<<endl;
		num += _clause_status[_ternary_var_membership_lists[var][i]];
	}
	for ( unsigned i = 0; i < _quaternary_var_membership_lists[var].size(); i += 4 ) {
		cout<<"value of _quaternary_var_membership_lists is "<<_clause_status[_quaternary_var_membership_lists[var][i]]<<endl;
		num += _clause_status[_quaternary_var_membership_lists[var][i]];
	}
	for ( unsigned i = 0; i < _long_var_membership_lists[var].size(); i++ ) {
		cout<<"value of long_var_list is "<<_clause_status[_long_var_membership_lists[var][i]]<<endl;
		num += _clause_status[_long_var_membership_lists[var][i]];
	}
...
Variable Inprocessor::Pick_Good_Var_DLCP_Component( Component & comp )  // using clause_seen
{
	...
	for ( itr = comp.VarIDs_Begin() + 1, end = comp.VarIDs_End(); itr < end; itr++ ) {
		...
		find_Y = true;
		double itr_score = _lit_scores[*itr + *itr] * _lit_scores[*itr + *itr + 1];
		if ( itr_score > best_score || !belong_XORY[*best]) {
			best = itr;
			best_score = _lit_scores[*best + *best] * _lit_scores[*best + *best + 1];
		}
	}
...
void Inprocessor::Pick_Models( vector<Model *> & source, Literal lit, vector<Model *> & target )
{
	...
	// if (!source.empty()) cout<<"source no wrong"<<endl;
	// if (!target.empty()) cout<<"target is ok"<<endl;
