...
PSE PSE_parameters( "PSE" );

Counter_Parameters counter_parameters( "PSE" ); 
...
Tool_Parameters * tools[] = { & PSE_parameters, \
	&counter_parameters, \
...
void Print_Usage()
{
	//cout<<"?? "<<parameters.procedure_name<<endl;
	strcpy(parameters.procedure_name,"PSE"); 
	...
	cout << tools[0]->Tool_Name() << endl; // thl add
...
void Test_Counter()
{
	ECounter::Test( parameters.cnf_file, PSE_parameters, parameters.quiet );
}
...
void Test()
{
    if ( !parameters.quiet ) {
		if ( counter_parameters.competition ) {
			...
			// string filename = "",tmp = "";
			// for (int i = 0 ; i < strlen(parameters.cnf_file) ; i ++) {
			// 	if (parameters.cnf_file[i] == '.' && parameters.cnf_file[i + 1] == 'c') break;
			// 	if (tmp == "./benchmarks/") {
			// 		filename += parameters.cnf_file[i];
			// 	}
			// 	else tmp += parameters.cnf_file[i];
			// }
			// cout<<filename<<",";
			
			//cout << parameters.cnf_file << ",";
