...
protected:
	...
	void Allocate_and_Init_Auxiliary_Memory( Variable max_var )  // ToDo: whether can we optimize when mv < max_var
	{
		...
		_var_stamps = new unsigned [max_var + 1 + 20];
		_reasons = new Reason [max_var + 1 + 20];
		_dec_stack = new Literal [NumVars( max_var ) * 2];
		...
		_dec_offsets = new unsigned [NumVars( max_var ) + 2];
		// precode
		*/ 
	...
	void Assign( Literal lit, Reason reason = Reason::undef )
	{
		...
		Literal Lit = reason.Lit_Value();
		// cout<<"var "<<lit.Var()<<" assign "<<lit.Sign();
		// if (reason != Reason::undef) cout<<"  reason = "<<Lit.Var();
		// cout<<endl;
	}
...
protected:
	...
	vector<Clause> _long_clauses;// 存储长度>2的子句
...
protected:
	...
	bool Solve_SAT( vector<Model *> & models , vector< vector<int> > & clauses ); // thl add 求解SAT
