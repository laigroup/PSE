...
bool Solver::Load_Instance( CNF_Formula & cnf )
{
	...
	for ( i = 0; i < cnf.Num_Clauses(); i++ ) {  /// NOTE: load non-unary clauses here
		...
		if ( _clause_status[i] ) _clause_status[i] = false;
			...
			_long_clauses.push_back( clause.Copy() );  /// cannot use clause._lits because it will be free in ~CNF_Formula
		}
	}
...
void Solver::Init_Heur_Decaying_Sum()
{
	...
	for ( ; itr < end; itr++ ) {
		...
		if (itr->Size() > 2) _heur_decaying_sum[(*itr)[2]] += 1;
...
Reason Solver::BCP( unsigned start )
{
	...
	for ( ; start < _num_dec_stack; start++ ) {
		...
		for ( i = 0, size = watched.size(); i < size; ) {
			...
			// cout<<"lit = "<<lit<<"  the clause id = "<<watched[i]<<endl; // toremove
			//clause.Debug_Display(); // TO REMOVE DEBUG
			if ( clause.Size() < 3 ) {
...
void Solver::Un_BCP( unsigned start )
{
	while ( _num_dec_stack > start ) {
		// unsigned  var = _dec_stack[--_num_dec_stack].Var();
		// _assignment[var] = lbool::unknown;
		//if (!belong_XORY[var]) _assignment[var] = lbool::unknown;
		//out<<" unbcp var is "<<_dec_stack[_num_dec_stack- 1].Var()<<endl;
		_assignment[_dec_stack[--_num_dec_stack].Var()] = lbool::unknown; // pre code
	}
}
...
bool Solver::Solve_SAT( vector<Model *> & models , vector< vector<int> > & clauses)
{
	StopWatch watch;
	size_t i;
